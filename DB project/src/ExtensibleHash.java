...
public class ExtensibleHash implements Serializable {
	...
	public void insertIndex(String index) {
		...
		for (int i = 0; i < prefix.size() && !inserted; i++) {
			if (prefix.get(i)
					.equals(splitIndex[0].substring(0, prefix.get(i).length()))) {
				if (buckets.get(i).bucket.size() < BUCKET_SIZE) {
					for(int j =0;j< splitIndex.length;j++ )
					{
						concatIndex = concatIndex + "," + splitIndex[i]; 
					}
					buckets.get(i).bucket.add(concatIndex);
					inserted = true;
				} else {
					elongate(i);
					refresh(splitIndex[0]);
					break;
				}
			}
		}
